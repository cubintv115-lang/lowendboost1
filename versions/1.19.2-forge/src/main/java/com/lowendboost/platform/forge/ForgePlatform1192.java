...
/**
 * Platform implementation cho Minecraft 1.19.2 + Forge 43.x.
 * <p>
 * Khác biệt so với 1.20.1: API {@code Options.renderDistance()} đổi từ
 * {@code get()/set()} sang truy cập trực tiếp field {@code renderDistance}
 * (vì 1.19.x chưa chuyển sang {@code Option<T>}).
 */
public class ForgePlatform1192 implements Platform {

    private static final Logger LOG = Logger.getLogger(ForgePlatform1192.class.getName());
    private static final String MC_VERSION = "1.19.2";

    private final Minecraft mc;

    public ForgePlatform1192() {
    ...
    public void setParticleSetting(int level) {
        ...
        mc.options.particles = clamped == 0 ? net.minecraft.client.ParticleStatus.ALL :
                clamped == 1 ? net.minecraft.client.ParticleStatus.DECREATED :
    ...
    public int disableDistantEntityAnimations(int distanceBlocks) {
        ...
        for (Entity entity : mc.level.entitiesForRendering()) {
            ...
            if (entity.distanceToSqr(playerPos) > distSq) {
                count++;
            }
        }
        return count;
    ...
    public void applyConfig(LowEndBoostConfig config, HardwareInfo hardware) {
        ...
        LOG.info(String.format("[LowEndBoost/1.19.2] Applied config: renderDist=%d, particles=%.2f, maxFps=%d",
